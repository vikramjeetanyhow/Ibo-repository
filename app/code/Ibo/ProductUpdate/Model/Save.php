    ...
    public function execute()
    {
        ...
        if ($data) {
            try {
                ...
                if(isset($data['product']['package_weight_in_kg']) && !empty($data['product']['package_weight_in_kg'])){
                    $package_weight_in_kg = $data['product']['package_weight_in_kg'];
                    if((!is_numeric($package_weight_in_kg)) && (!is_float($package_weight_in_kg))){
                        $resultRedirect->setPath('catalog/*/edit',['id' => $product->getId(), 'back' => null, '_current' => true]);
                        $this->messageManager->addErrorMessage('package_weight_in_kg is not numeric and decimal.');
                        return $resultRedirect;
                    }
                }
                
                if(isset($data['product']['package_height_in_cm']) && !empty($data['product']['package_height_in_cm'])){
                    $package_height_in_cm = $data['product']['package_height_in_cm'];
                    if((!is_numeric($package_height_in_cm)) && (!is_float($package_height_in_cm))){
                        $resultRedirect->setPath('catalog/*/edit',['id' => $product->getId(), 'back' => null, '_current' => true]);
                        $this->messageManager->addErrorMessage('package_height_in_cm is not numeric and decimal');
                        return $resultRedirect;
                    }
                }
                
                if(isset($data['product']['package_length_in_cm']) && !empty($data['product']['package_length_in_cm'])){
                    $package_length_in_cm = $data['product']['package_length_in_cm'];
                    if((!is_numeric($package_length_in_cm)) && (!is_float($package_length_in_cm))){
                        $resultRedirect->setPath('catalog/*/edit',['id' => $product->getId(), 'back' => null, '_current' => true]);
                        $this->messageManager->addErrorMessage('package_length_in_cm is not numeric and decimal');
                        return $resultRedirect;
                    }
                }
                
                if(isset($data['product']['package_width_in_cm']) && !empty($data['product']['package_width_in_cm'])){
                    $package_width_in_cm = $data['product']['package_width_in_cm'];
                    if((!is_numeric($package_width_in_cm)) && (!is_float($package_width_in_cm))){
                        $resultRedirect->setPath('catalog/*/edit',['id' => $product->getId(), 'back' => null, '_current' => true]);
                        $this->messageManager->addErrorMessage('package_width_in_cm is not numeric and decimal');
                        return $resultRedirect;
                    }
                }   
               
                if(isset($data['product']['hsn_code']) && !empty($data['product']['hsn_code'])){
                    $hsn_code = $data['product']['hsn_code'];
                    if((!is_numeric($hsn_code))){
                        $resultRedirect->setPath('catalog/*/', ['store' => $storeId]);
                        $resultRedirect->setPath('catalog/*/edit',['id' => $product->getId(), 'back' => null, '_current' => true]);
                        $this->messageManager->addErrorMessage('hsn_code is not numeric');
                        return $resultRedirect;
                    }
                }

                if(isset($data['product']['lot_control_parameters'])  && !empty($data['product']['lot_control_parameters'])){
                    $lot_control_parameters = strtoupper($data['product']['lot_control_parameters']);
                    $lotdata = array('MRP', 'BATCH', 'MRP,BATCH','BATCH,MRP','GRN_Date_YYMMDD');
                    if (!in_array($lot_control_parameters, $lotdata)){
                        $resultRedirect->setPath('catalog/*/', ['store' => $storeId]);
                        $resultRedirect->setPath('catalog/*/edit',['id' => $product->getId(), 'back' => null, '_current' => true]);
                        $this->messageManager->addErrorMessage('lot_control_parameters data is not correct');
                        return $resultRedirect;
                    }
                }
