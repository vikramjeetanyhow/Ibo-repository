...
class UpdatePriceHandler extends AbstractHandler
{
    ...
    private function updateValues(array $valuesToUpdate, array $oldValues): bool
    {
        ...
        foreach ($valuesToUpdate as $key => $value) {
           if ((!empty($value['value']) && (float)$oldValues[$key]['price'] !== (float)$value['value'])
               ...
               || $this->getCustomerZone($oldValues[$key]) !== $this->getCustomerZone($value)
           ) {
               $price = new \Magento\Framework\DataObject(
                   [
                       ...
                       'customer_zone' => $this->getCustomerZone($value),
                   ]
               );
    ...
    private function getPriceKey(array $priceData): string
    {
        ...
        $key = implode(
            '-',
            array_merge([$priceData['website_id'], $priceData['cust_group'],$priceData['customer_zone']], [$qty])
        );
