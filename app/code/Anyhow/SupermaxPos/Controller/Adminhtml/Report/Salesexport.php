...
class Salesexport extends \Magento\Backend\App\Action
{
    ...
    public function __construct(
        ...
        \Anyhow\SupermaxPos\Model\ResourceModel\SupermaxSalesOrders\Collection $registerData,
    ...
    {   
        ...
        if(!empty($storeCurrencyCode)) {
            $storeCurrencySymbol = $this->currency->getCurrency($storeCurrencyCode)->getName();
        }
        $name = date('m-d-Y-H-i-s');
        $filepath = 'export/sales-report-import-' .$name. '.csv';
        $this->directory->create('export');

        $stream = $this->directory->openFile($filepath, 'w+');
        $stream->lock();

        $columns = ['Date Start','Date End', 'No. Orders', 'No. Products', 'Tax ('.$storeCurrencyCode .')', 'Total ('.$storeCurrencyCode .')'];
        ...
        $csvfilename = 'sales-report-import-'.$name.'.csv';
