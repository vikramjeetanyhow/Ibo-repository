            ...
            if (!count($postItems)) {
				...
				foreach (array_keys($postItems) as $entityId) 
				{
                    ...
                    try {
                        ...
                        $headCashierPartialBalanceTotal = preg_replace('/[^0-9\.]/', "", $postItems[$entityId]['head_cashier_partial_balance_total']);
                        $headCashierNote = $postItems[$entityId]['head_cashier_close_note'];
                
						$query = $connection->update($registerTable,
                            ['head_cashier_cash_total'=> $headCashierCashTotal, 'head_cashier_card_total'=> $headCashierCardTotal, 'head_cashier_custom_total'=> $headCashierCustomTotal, 'head_cashier_offline_total'=> $headCashierOfflineTotal, 'head_cashier_partial_balance_total'=> $headCashierPartialBalanceTotal, 'head_cashier_close_note'=> $headCashierNote, 'reconciliation_status' => 1], $where);
                    } catch (\Exception $e) {
