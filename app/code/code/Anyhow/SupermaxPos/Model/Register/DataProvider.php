...
class DataProvider extends \Magento\Ui\DataProvider\AbstractDataProvider
{
    ...
    public function getData()
    {
        ...
        foreach ($items as $registerdata) {
            ...
            $this->loadedData[$registerdata->getId()]['cashexpected'] = round($cashExpected, 2);
            $this->loadedData[$registerdata->getId()]['cashtotal'] = round($cashTotal, 2);
            $this->loadedData[$registerdata->getId()]['cashdifference'] = round($cashDifference, 2);
            $this->loadedData[$registerdata->getId()]['cardexpected'] = round($cardExpected, 2);
            $this->loadedData[$registerdata->getId()]['cardtotal'] = round($cardTotal, 2);
            $this->loadedData[$registerdata->getId()]['carddifference'] = round($cardDifference, 2);
            $this->loadedData[$registerdata->getId()]['customexpected'] = round($customExpected, 2);
            $this->loadedData[$registerdata->getId()]['customtotal'] = round($customTotal, 2);
            $this->loadedData[$registerdata->getId()]['customdifference'] = round($customDifference, 2);
            $this->loadedData[$registerdata->getId()]['walletexpected'] = round($walletExpected, 2);
            $this->loadedData[$registerdata->getId()]['wallettotal'] = round($walletTotal, 2);
            $this->loadedData[$registerdata->getId()]['walletdifference'] = round($walletDifference, 2);
            $this->loadedData[$registerdata->getId()]['offlineexpected'] = round($offlineExpected, 2);
            $this->loadedData[$registerdata->getId()]['offlinetotal'] = round($offlineTotal, 2);
            $this->loadedData[$registerdata->getId()]['offlinedifference'] = round($offlineDifference, 2);
            $this->loadedData[$registerdata->getId()]['emiexpected'] = round($emiExpected, 2);
            $this->loadedData[$registerdata->getId()]['emitotal'] = round($emiTotal, 2);
            $this->loadedData[$registerdata->getId()]['emidifference'] = round($emiDifference, 2);
            $this->loadedData[$registerdata->getId()]['bankdepositexpected'] = round($bankDepositExpected, 2);
            $this->loadedData[$registerdata->getId()]['bankdeposittotal'] = round($bankDepositTotal, 2);
            $this->loadedData[$registerdata->getId()]['bankdepositdifference'] = round($bankDepositDifference, 2);
            $this->loadedData[$registerdata->getId()]['paylaterexpected'] = round($payLaterExpected, 2);
            $this->loadedData[$registerdata->getId()]['paylatertotal'] = round($payLaterTotal, 2);
            $this->loadedData[$registerdata->getId()]['paylaterdifference'] = round($payLaterDifference, 2);
            $this->loadedData[$registerdata->getId()]['partialballanceexpected'] = round($partialBallanceExpected, 2);
            $this->loadedData[$registerdata->getId()]['partialballancetotal'] = round($partialBallanceTotal, 2);
            $this->loadedData[$registerdata->getId()]['partialballancedifference'] = round($partialBallanceDifference, 2);
