...
class OfflineTotal extends Column
{
    /**
    ...
    public function prepareDataSource(array $dataSource)
    {
        ...
        if (isset($dataSource['data']['items'])) {
            ...
            foreach ($dataSource['data']['items'] as &$item) {
                ...
                $getRegisterTotal = $connection->query("SELECT SUM(total) as total_total FROM $supermaxRegisterTransTable WHERE pos_register_id = '" . $posRegisterId . "' AND code = 'offline' ");
                foreach ($getRegisterTotal as $RegisterTotal) {
                    $total = (float) $RegisterTotal['total_total'];
                }
                $item[$fieldName] = $storeCurrencySymbol . round((float)$total, 2);
            }
        }
        return $dataSource;
    }
