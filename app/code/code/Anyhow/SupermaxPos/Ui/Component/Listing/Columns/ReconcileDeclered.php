...
class ReconcileDeclered extends Column
{
    ...
    {       
        ...
        if (isset($dataSource['data']['items'])) {
            $cashExpected = $cardExpected = $customExpected = $offlineExpected = $floatamount = $walletExpected = $emiExpected = $bankDepositExpected = $payLatertExpected = $partialBalanceExpected = 0.00;
            $storeCurrencySymbol = '';
            ...
            $getPartialBalanceTotalExpected = $connection->query("SELECT SUM(total) as total_total FROM $supermaxRegisterTransTable WHERE pos_register_id = '" .$posRegisterId. "' AND code = 'partial_payment_balance' ");
            foreach ($getPartialBalanceTotalExpected as $getPartialBalanceExpected) {
                $partialBalanceExpected = (float) $getPartialBalanceExpected['total_total'];
            }
        }
        $i = 0;
        // if (isset($dataSource['data']['items'])) {
            ...
            foreach ($dataSource['data']['items'] as &$item) {
                if($item['paymentCode'] == 'cash') {
                    ...
                    $item[$fieldName] = $storeCurrencySymbol . round($cashExpected, 2);
                } elseif($item['paymentCode'] == 'offline') {
                    $item['pos_register_id'] = $posRegisterId;
                    $item[$fieldName] = $storeCurrencySymbol . round($offlineExpected, 2);
                } elseif($item['paymentCode'] == 'card') {
                    $item['pos_register_id'] = $posRegisterId;
                    $item[$fieldName] = $storeCurrencySymbol . round($cardExpected, 2);
                } elseif($item['paymentCode'] == 'upi') {
                    $item['pos_register_id'] = $posRegisterId;
                    $item[$fieldName] = $storeCurrencySymbol . round($customExpected, 2);
                } elseif($item['paymentCode'] == 'wallet') {
                    $item['pos_register_id'] = $posRegisterId;
                    $item[$fieldName] = $storeCurrencySymbol . round($walletExpected, 2);
                } elseif($item['paymentCode'] == 'emi') {
                    $item['pos_register_id'] = $posRegisterId;
                    $item[$fieldName] = $storeCurrencySymbol . round($emiExpected, 2);
                } elseif($item['paymentCode'] == 'bank_deposit') {
                    $item['pos_register_id'] = $posRegisterId;
                    $item[$fieldName] = $storeCurrencySymbol . round($bankDepositExpected, 2);
                } elseif($item['paymentCode'] == 'pay_later') {
                    $item['pos_register_id'] = $posRegisterId;
                    $item[$fieldName] = $storeCurrencySymbol . round($payLatertExpected, 2);
                } elseif($item['paymentCode'] == 'partial_payment_balance') {
                    $item['pos_register_id'] = $posRegisterId;
                    $item[$fieldName] = $storeCurrencySymbol . round($partialBalanceExpected, 2);
                }
               $i++;
            }
