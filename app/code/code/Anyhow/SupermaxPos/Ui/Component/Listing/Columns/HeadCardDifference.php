...
class HeadCardDifference extends Column
{
    /**
    ...
    {   
        ...
        if(isset($dataSource['data']['items'])) {
            ...
            foreach($dataSource['data']['items'] as & $item) {
                ...
                $getRegisterData = $connection->query("SELECT head_cashier_card_total, (SELECT SUM(amount) as expected_total FROM $supermaxRegisterTransDetailTable WHERE pos_register_id = '" .$posRegisterId. "' AND code = 'card') AS card_total FROM $supermaxRegister WHERE pos_register_id = '" . $posRegisterId . "'")->fetch();
              
                if(!empty($getRegisterData)) {
                    $difference = $getRegisterData['head_cashier_card_total'] - $getRegisterData['card_total'];
                }
