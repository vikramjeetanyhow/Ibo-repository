...
class OfflineExpected extends Column
{
    /**
    ...
    {      
        ...
        if(isset($dataSource['data']['items'])) {
            ...
            foreach($dataSource['data']['items'] as & $item) {
                ...
                $getRegistertotalExpected = $connection->query("SELECT SUM(amount) as expected_total FROM $supermaxRegisterTransDetailTable WHERE pos_register_id = '" .$posRegisterId. "' AND code = 'offline' ");

                foreach($getRegistertotalExpected as $getExpectedTotal) {
                    $expected = (float)$getExpectedTotal['expected_total'];
                }

                $item[$fieldName] = $storeCurrencySymbol.round((float)$expected, 2);
            }
        }
        return $dataSource;
    }
