...
class HeadCashDifference extends Column
{
    /**
    ...
    {   
        ...
        if(isset($dataSource['data']['items'])) {
            ...
            foreach($dataSource['data']['items'] as & $item) {
                ...
                $getRegisterData = $connection->query("SELECT head_cashier_cash_total, (SELECT SUM(amount) as expected_total FROM $supermaxRegisterTransDetailTable WHERE pos_register_id = '" .$posRegisterId. "' AND code = 'cash') AS cash_total FROM $supermaxRegister WHERE pos_register_id = '" . $posRegisterId . "'")->fetch();
              
                if(!empty($getRegisterData)) {
                    $difference = $getRegisterData['head_cashier_cash_total'] - $getRegisterData['cash_total'];
                }
