...
class ReconcileDiffrence extends Column
{
    ...
    public function prepareDataSource(array $dataSource)
    {
        ...
        if (isset($dataSource['data']['items'])) {
            ...
            $getRegisterDataPartialBalance = $connection->query("SELECT head_cashier_partial_balance_total, (SELECT SUM(amount) as expected_total FROM $supermaxRegisterTransDetailTable WHERE pos_register_id = '" . $posRegisterId . "' AND code = 'partial_payment_balance') AS partial_payment_balance_total FROM $supermaxRegister WHERE pos_register_id = '" . $posRegisterId . "'")->fetch();

            if (!empty($getRegisterDataPartialBalance)) {
                $partialBallanceDifference = $getRegisterDataPartialBalance['partial_payment_balance_total'] - $getRegisterDataPartialBalance['head_cashier_partial_balance_total'];
            }
        }
        $i = 0;
        foreach ($dataSource['data']['items'] as &$item) {
            if($item['paymentCode'] == 'cash') {
                $item[$fieldName] = $storeCurrencySymbol . round($cashDifference, 2);
            } elseif($item['paymentCode'] == 'offline') {
                $item[$fieldName] = $storeCurrencySymbol . round($offlineDifference, 2);
            } elseif($item['paymentCode'] == 'card') {
                $item[$fieldName] = $storeCurrencySymbol . round($cardDifference, 2);
            } elseif($item['paymentCode'] == 'upi') {
                $item[$fieldName] = $storeCurrencySymbol . round($customDifference, 2);
            } elseif($item['paymentCode'] == 'wallet') {
                $item[$fieldName] = $storeCurrencySymbol . round($walletDifference, 2);
            } elseif($item['paymentCode'] == 'emi') {
                $item[$fieldName] = $storeCurrencySymbol . round($emiDifference, 2);
            } elseif($item['paymentCode'] == 'bank_deposit') {
                $item[$fieldName] = $storeCurrencySymbol . round($bankDepositDifference, 2);
            } elseif($item['paymentCode'] == 'pay_later') {
                $item[$fieldName] = $storeCurrencySymbol . round($payLaterDifference, 2);
            } elseif($item['paymentCode'] == 'partial_payment_balance') {
                $item[$fieldName] = $storeCurrencySymbol . round($partialBallanceDifference, 2);
            }
            $i++;
        }
