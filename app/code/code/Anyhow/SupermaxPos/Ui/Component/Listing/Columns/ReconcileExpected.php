...
class ReconcileExpected extends Column
{
    ...
    public function prepareDataSource(array $dataSource)
    {
        ...
        $title = "Opening Float";
        $cashExpected = $cardExpected = $customExpected = $offlineExpected = $floatamount = $walletExpected = $emiExpected = $bankDepositExpected = $payLaterExpected = $partialBalanceExpected = 0;
        ...
        if (isset($dataSource['data']['items'])) {
            ...
            if (!empty($getCustomTotalExpected)) {
                foreach ($getCustomTotalExpected as $getCustomExpected) {
                    ...
                    } elseif ($getCustomExpected['code'] == 'partial_payment_balance') {
                        $partialBalanceExpected += (float) $getCustomExpected['amount'];
                    }
                }
            }
        }
        ...
        foreach ($dataSource['data']['items'] as &$item) {
            if($item['paymentCode'] == 'cash') {
                ...
                $item[$fieldName] = $storeCurrencySymbol . round($payLaterExpected, 2);
