...
class CustomerOrders implements ResolverInterface
{
    ...
    public function resolve(
        ...
        $promise_expires_at = '';
        $orderChannel = '';
            foreach($searchResult->getData() as $orderData){
            $status_code = isset($orderData['status']) ? $orderData['status'] : '';
            $createdDate = isset($orderData['created_at']) ? $orderData['created_at'] : '';
            $orderChannel = isset($orderData['order_channel']) ? $orderData['order_channel'] : '';
            if(!empty($orderData['promise_options'])){
                $promiseOptions = json_decode($orderData['promise_options'], true);
                foreach($promiseOptions as $newkey=>$promiseOption){
                    ...
                    $shippingDetails['estimated_shipping_date'] = $promiseDeliveryInfo['delivery_slot']['to_date_time'];
                    
                    }
                    }
                    }
            }
            $promise_created_at = isset($orderData['promise_created_at']) ? $orderData['promise_created_at'] : '';
            $promise_expires_at = isset($orderData['promise_expires_at']) ? $orderData['promise_expires_at'] : '';
        } 
        if(isset($shippingDetails['estimated_shipping_date']) && !empty($shippingDetails['estimated_shipping_date'] )){
            $createdDate = $shippingDetails['estimated_shipping_date'];
        }
        return [
            ...
            'promise_created_at' => $promise_created_at,
            'promise_expires_at' => $promise_expires_at,
