...
class TotalDiscounts implements ResolverInterface
{
    ...
    {        
        ...
        foreach ($order->getAllVisibleItems() as $_item) {
            ...
            $additionalDiscount = $_item->getDiscountAmount();
            $itemQty = $_item->getQtyOrdered();
            $sellingPrice = $itemBasePriceSingleQty * $itemQty;
            $product = $this->productRepository->getById($productId);
            $itemMRPTotal = $product->getMrp() * $itemQty;
            if($sellingPrice > $itemMRPTotal) {
                $mrpItemDiscount = 0;
            }else{
                $mrpItemDiscount = $itemMRPTotal - $sellingPrice;
            }
            $itemTotalDiscount += $mrpItemDiscount + $additionalDiscount;
        }
        $mrpDiscount['value'] = $itemTotalDiscount;
