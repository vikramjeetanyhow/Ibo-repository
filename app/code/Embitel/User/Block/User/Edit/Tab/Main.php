...
namespace Embitel\User\Block\User\Edit\Tab;
...
class Main extends \Magento\User\Block\User\Edit\Tab\Main
{
    ...
    protected function _prepareForm()
    {
        ...
        $baseFieldset->addField(
            ...
            [
                'name' => 'pos_store_role',
                'label' => __('POS Store Role '),
                'title' => __('POS Store Role '),
                'values' => $this->getOutletData(),
                'class' => 'select'
            ]
        );
    ...
    protected function getOutletData()
    {
        $connection = $this->resource->getConnection();
        $outletTable = $this->resource->getTableName('ah_supermax_pos_outlet');
        $outletData = $connection->query("SELECT * FROM $outletTable")->fetchAll(); 
        $options = array();
        $options[] = ['value' => "0", 'label' => 'All Stores'];
        if($outletData) { 
            foreach($outletData as $data) { 
                $options[] = ['value' => $data['pos_outlet_id'], 'label' => $data['outlet_name']." - ".$data['pos_outlet_id']];
            }
        } 
        return $options;
    }    
