...
class Utility extends \Magento\SalesRule\Model\Utility
{
     ...
     */
    public function canProcessRule($rule, $address): bool
    {
        if ($rule->hasIsValidForAddress($address) && !$address->isObjectNew()) {
            return $rule->getIsValidForAddress($address);
        }
        $availableChannels = explode(',', $rule->getCouponUseIn());

        $channel = $address->getQuote()->getChannel();

        if ($channel === 'store') {
            $channel = $address->getQuote()->getChannelInfo();
        }

        if (!in_array('all', $availableChannels)) {
