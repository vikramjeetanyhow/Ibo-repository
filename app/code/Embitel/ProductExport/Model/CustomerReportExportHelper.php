...
class CustomerReportExportHelper
{
    const LOG_PATH = '/var/log/ebo_product_export_report.log';
    const XML_PATH_CRON_STATUS = 'eboexport/ebo_customer_report_config/cron_status';
    ...
    public function isCronEnabled(): bool
    {
        ...
        if (!$isCronEnable || $isCronEnable == 0) {
            $this->addLog('Cron to export customer disabled');
        }
        return $isCronEnable;
    }
    ...
    function getConfigs(): DataObject
    {
        if (!$this->configs->getCronStatus()) {
            $this->configs->addData(
                $this->scopeConfig->getValue('eboexport/ebo_customer_report_config', ScopeInterface::SCOPE_WEBSITE)
            );
        }
        return $this->configs;
    }
