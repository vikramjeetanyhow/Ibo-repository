 ...
 */
namespace Embitel\ProductFilter\Elasticsearch\SearchAdapter\Filter;
 ...
 */
class Builder extends \Magento\Elasticsearch\SearchAdapter\Filter\Builder
{
    /**
     * @var FilterInterface[]
     */
    ...
    public function __construct(
        ...
        parent::__construct($range, $term, $wildcard);
    }
    ...
    private function processFilter(RequestFilterInterface $filter, $conditionType)
    {
        if (RequestFilterInterface::TYPE_BOOL == $filter->getType()) {
            ...
            if ($filter->getType() == RequestFilterInterface::TYPE_RANGE && $filter->getField() == 'price') {
                $query = [
                    'bool' => [
                        'should' => $this->filters[$filter->getType()]->buildFilter($filter),
                    ]
                ];
                //end
            } else {
