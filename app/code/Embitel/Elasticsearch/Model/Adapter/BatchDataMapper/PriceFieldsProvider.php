...
namespace Embitel\Elasticsearch\Model\Adapter\BatchDataMapper; 
 ...
 */
class PriceFieldsProvider extends \Magento\Elasticsearch\Model\Adapter\BatchDataMapper\PriceFieldsProvider
{ 
    /**
     * @var Index
     */
    ...
    public function __construct(
        ...
        \Embitel\AdvancedSearch\Model\ResourceModel\Index $resourceIndexCustom,
    ...
    { 
        ...
        $priceRegionSwitch = $this->scopeConfigInterface->getValue("regional_pricing/setting/active"); 
        if($priceRegionSwitch == 1) { 
            $priceData = $this->dataProvider->getSearchableAttribute('price')
            ? $this->resourceIndexCustom->getPriceIndexData($productIds, $storeId)
            : [];
        } else { 
            $priceData = $this->dataProvider->getSearchableAttribute('price')
    ...
    private function getProductPriceData($productId, $websiteId, array $priceIndexData)
    {
        ...
        if (array_key_exists($productId, $priceIndexData)) {
            ...
            if(array_key_exists('tier_price', $productPriceIndexData)) {
                unset($productPriceIndexData['tier_price']);
            }
